...
## SPECIFICATION RECTIFICATION — PRODUCTION GRADE RECONCILIATION — 2026-08-23
...
TARGET_API    = mod_forum\grades\forum_gradeitem::store_grade_from_formdata()
...
FORUM_GRADES_NON_NULL_GRADE_COUNT = 0  (NO "vacío": existen filas placeholder NULL)

INVARIANTES OBLIGATORIOS:
  FRESH_GRADE_COUNT_PRE = N · GRADE_COUNT_POST = FRESH_GRADE_COUNT_PRE · VALUE_IDENTITY_REQUIRED = YES
  SOURCE_FINALGRADE_PRE = forum_grades.grade POST = grade_grades.finalgrade POST
  ...
  UNEXPECTED_FORUM_GRADE_ROWS = 0 · UNEXPECTED_NON_NULL_FORUM_GRADES = 0

EXECUTION MODEL:
  EXECUTION_ORDER = PER_FORUM · SAFE_WRITE_UNIT = PER_GRADE
  STOP_ON_FIRST_FAILURE = YES
  TRANSACTION_BOUNDARY = NONE_EXPLICIT (API WRITE + double read-back + stop-on-failure)

ROLLBACK:
  LEGACY_EXACT_LOGICAL_ROLLBACK = NOT_DEMONSTRATED
  SAFE_PARTIAL_RECONCILIATION = YES (solo si cada safe unit verificó SOURCE_PRE=FORUM_POST=GRADEBOOK_POST)
  FULL_DB_RESTORE = ONLY_FOR_CONFIRMED_DATA_CORRUPTION AND EXPLICIT_USER_AUTHORIZATION
...
PROHIBICIONES:
  ...
  DIRECT INSERT/UPDATE forum_grades/grade_grades = NO
